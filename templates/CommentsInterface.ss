...
<% if $CommentsEnabled %>
    <div id="$CommentHolderID" class="comments-holder-container">
        <h4><%t CommentsInterface_ss.POSTCOM "Post your comment" %></h4>
        ...
        <% if $AddCommentForm %>
            <% if $canPostComment %>
                ...
                $AddCommentForm
            <% else %>
                <p><%t CommentsInterface_ss.COMMENTLOGINERROR "You cannot post comments until you have logged in" %><% if $PostingRequiredPermission %>, <%t CommentsInterface_ss.COMMENTPERMISSIONERROR "and have an appropriate permission level" %><% end_if %>.
        ...
        <h4><%t CommentsInterface_ss.COMMENTS "Comments" %></h4>
